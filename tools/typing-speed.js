...
window.renderTypingSpeed = function(el) {
  ...
  // Init sentence mode
  tsSetMode('en');
  // Init word mode (generates words even though hidden)
  tgSetMode('en');
  // Init code mode
  tcReset();
  // Render stats
  ...
  // Cleanup all timers when navigating away
...
function tgHandleInput() {
  ...
  // Space/Enter submits the current word
...
function tcHandleInput() {
  ...
  // Render target with char-by-char coloring
  let html = '';
  let correct = 0;
  ...
  if (typed.length >= target.length) {
    clearInterval(s.timer);
    s.running = false;
    // Convert CPM to WPM equivalent for storage (CPM / 5)
...
function typRenderStats() {
  ...
  // SVG line chart of last 20 scores
  if (!chartEl) return;
...
// _activeCleanup is set inside renderTypingSpeed (see end of render function)

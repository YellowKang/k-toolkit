/* ── Calculator — Basic + Scientific Mode + Expression History ── */
...
function renderCalculator(el) {
  ...
  /* ── press handler ── */
  function press(k) {
    // Scientific function buttons insert text into expression
    const sciFns = {
    ...
    if (k === 'C') {
      ...
      // Insert function: append fn( to expression, reset val
      ...
      // Append current val and closing paren
      expr = (expr || '') + val + ')';
      val = '0'; newNum = true; lastResult = null;
    }
    else if ('÷×−+'.includes(k)) {
      ...
      try {
        ...
        // Replace math function names with Math.xxx
        if (degMode) {
          // Wrap trig args in degree-to-radian conversion
          ...
          // Close the extra parens: each trig fn now has sin((PI/180)*( ... ))
          // We need to find matching closing paren and add extra one
          // Simpler: post-process — wrap each trig call
          // Actually the user types sin(30) which becomes Math.sin((Math.PI/180)*(30))
          // The user's ) closes the original (, so we need one more )
          // Let's use a smarter approach: replace right before eval
  ...
  /* Helper: for DEG mode, adds extra closing paren for trig wrappers */
  function _closeDegParens(s) {
    // Pattern: Math.sin((Math.PI/180)*( ... ))
    // The user's expression has sin(X) which became Math.sin((Math.PI/180)*(X)
    // We need to add )) at the end of each trig call's closing paren
    // Strategy: find each Math.sin/cos/tan and track parens
    const result = [];
    let i = 0;
    while (i < s.length) {
      ...
      if (trigMatch) {
        ...
        result.push(prefix);
        i += prefix.length;
        // Now find the matching close paren for the user's (
        let depth = 1;
        ...
        result.push('))'); // close the two extra parens
      } else {
        result.push(s[i]);
        i++;
      }

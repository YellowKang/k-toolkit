function renderUnitConvert(el) {
  ...
  el.innerHTML = `
    <div class="tool-card-panel">
      ...
      <div id="ucNumFmtPanel" style="display:none">
        <div style="margin-bottom:12px">
          <div style="font-size:12px;color:var(--text-muted);margin-bottom:6px">输入数字</div>
          <input type="text" class="tool-input" id="ucNfInput" placeholder="输入数字，如 1234567.89" oninput="ucNfUpdate()">
        </div>
      </div>
    </div>
...
function ucSetCat(cat) {
  ...
  const isNumFmt = cat === '数字格式';
  const convertPanel = document.getElementById('ucConvertPanel');
  const numFmtPanel = document.getElementById('ucNumFmtPanel');
  if (convertPanel) convertPanel.style.display = isNumFmt ? 'none' : '';
  if (numFmtPanel) numFmtPanel.style.display = isNumFmt ? '' : 'none';
  if (isNumFmt) {
    ucNfUpdate();
    return;
  }
...
// ── 数字格式化（合入 number-format）──
function _ucNfRow(label, value) {
  const escaped = String(value).replace(/'/g, "\\'");
  return `<div class="result-row" style="margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid var(--glass-border);border-radius:10px">
    <span style="color:var(--text-muted);font-size:13px">${label}</span>
    <div style="display:flex;align-items:center;gap:10px">
      <code style="font-family:monospace;font-size:14px;color:var(--neon)">${value}</code>
      <button class="copy-inline" onclick="copyText('${escaped}',this)">复制</button>
    </div>
  </div>`;
}

function ucNfUpdate() {
  const raw = (document.getElementById('ucNfInput').value || '').trim().replace(/,/g,'');
  const allPanel = document.getElementById('ucAllPanel');
  const allResults = document.getElementById('ucAllResults');
  if (!raw || isNaN(+raw)) {
    allResults.innerHTML = '<span style="color:var(--text-muted);font-size:13px">输入数字后显示格式化结果</span>';
    return;
  }
  const n = +raw;
  const fmt = (v, locale, opts) => new Intl.NumberFormat(locale, opts).format(v);
  const rows = [
    _ucNfRow('千分位（中文）', fmt(n,'zh-CN')),
    _ucNfRow('千分位（英文）', fmt(n,'en-US')),
    _ucNfRow('人民币 ¥', fmt(n,'zh-CN',{style:'currency',currency:'CNY'})),
    _ucNfRow('美元 $', fmt(n,'en-US',{style:'currency',currency:'USD'})),
    _ucNfRow('欧元', fmt(n,'de-DE',{style:'currency',currency:'EUR'})),
    _ucNfRow('科学计数法', n.toExponential()),
  ...
  if (n >= 0) {
    ...
    rows.push(_ucNfRow('字节友好格式', (n/best[1]).toFixed(best[0]==='B'?0:2) + ' ' + best[0]));
  }
  allResults.innerHTML = rows.join('');
  const panelLabel = allPanel.querySelector('.panel-label');
  if (panelLabel) panelLabel.textContent = '格式化结果';
}

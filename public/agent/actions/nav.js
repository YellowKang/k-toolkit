...
// ── 工具能力描述表 ───────────────────────────────────────────────
// autoRun: true = AI 可直接填数据执行；false = 需要用户手动操作（文件上传类）
const TOOL_CAPS = {
  ...
  // autoRun:false 需要用户手动操作（文件上传、Canvas 交互类）
  ...
  // 以下工具有 TOOL_RUNNERS 或 SPECIAL_RUNNERS 配置，autoRun:true
...
// ── 特殊工具专用执行器 ───────────────────────────────────────────
// 这些工具不走通用 mainInput/trigger 流程，需要直接操作全局变量
const SPECIAL_RUNNERS = {
  'nginx-gen': async ({ inputs, action }) => {
    // inputs: { domain, scene, root, port, ssl, gzip, cache, accessLog, upstream, servers }
    // scene: static | spa | proxy | https | loadbalance
    ...
    // checkbox: ssl, gzip, cache, accessLog
    ...
    // Apply scene preset then regenerate
  ...
  'gradient': async ({ inputs }) => {
    // inputs: { colors: ['#hex',...], positions: [0,50,100], angle: 135, type: 'linear' }
  ...
  'shadow': async ({ inputs }) => {
    // Fill first layer from agent inputs if provided
...
const NavActions = [
  ...
  },
  {
    ...
    execute({ query, category }) {
      ...
      // 对每个工具评分：匹配的 token 越多、匹配位置越重要，得分越高
      const scored = [];
      ...
      // 搜索匹配的 agent actions（非 core tier），同样按 token 评分
  ...
  },
  {
    ...
    async execute({ toolId, inputs, action }) {
      // 等待页面切换完成（navigate_to_tool 走 hash 异步渲染）
      ...
      if (!runner) {
        // 没有配置但工具存在，尝试通用填值（找页面上第一个 textarea/input）
      ...
      // ── beforeRun 钩子：在填值前执行（用于切换 tab 等前置操作）──
      if (runner.beforeRun) {
        try { runner.beforeRun({ action, inputs }); } catch(e) { console.warn('beforeRun error:', e); }
        await new Promise(r => setTimeout(r, 100));
      }

      // ── actionOverrides：按 action 覆盖 runner 配置 ──
      // 覆盖后的 runner 拥有该 action 专属的 mainInput/trigger/outputSelector/extraInputs/waitMs
      ...
      // 等待工具页 DOM 渲染完成（navigateTo 是异步的）
      if (runner.mainInput) {
        const deadline = Date.now() + 2000;
      ...
      // 填入额外输入（如 regex pattern/flags，text-diff text2）
      ...
      // 前置操作（如 url-parser 需要先切换 tab）— 使用 baseRunner 的 preTriggerMap
...
function _showResultPanel(title, items, copyAll, toolId) {
  ...
  items.forEach((item, idx) => {
    ...
    if (itemType === 'color') {
      // Color swatch — sanitize to valid CSS color values only (hex, rgb, hsl, named)
